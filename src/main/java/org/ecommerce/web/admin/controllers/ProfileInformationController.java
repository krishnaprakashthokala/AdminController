...
import org.ecommerce.config.web.services.UserService;
...
public class ProfileInformationController {
	...
	public String show(@CurrentUser User currentUser, Model model) {
		User user = null;//krishna//userRepository.findOne(currentUser.getId());
	...
	public String process(
			// @ModelAttribute will load User from session but also set values from the form
			// post
			@Validated(User.UserUpdate.class) @ModelAttribute(ATTRIBUTE_NAME) User user, BindingResult bindingResult,
			RedirectAttributes redirectAttributes,
			// SessionStatus lets you clear your SessionAttributes
			SessionStatus sessionStatus, @CurrentUser User currentUser) {
		...
		successMessages
				.add(messageSource.getMessage("message.profile.save.success", new Object[] {}, Locale.getDefault()));
