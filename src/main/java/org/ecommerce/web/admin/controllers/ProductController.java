...
import org.ecommerce.config.web.admin.exceptions.ProductNotFoundException;
import org.ecommerce.config.web.admin.exceptions.ProductsNotFoundException;
...
public class ProductController {
	...
	@GetMapping("/report")
	public ModelAndView report(@Valid @Pattern(regexp = "pdf|excel") @RequestParam("format") String format,
		...
		modelMap.put("logo",
				ClassLoader.getSystemClassLoader().getResourceAsStream("jasperreports/images/invoice_logo.png"));
	...
	public String create(Model model) {
		...
		logger.info("create  admin Order Controller  create: " + model);
	...
	public String edit(@PathVariable Long productId, Model model) {
		...
		if (!model.containsAttribute(BINDING_RESULT_NAME)) {
			Product product = null; //krishna// productsRepository.findOne(productId);
			if (product == null) {
		...
		logger.info("edit  admin Order Controller  create: " + productId);
	...
	public String process(
			// Flash flash,
			@ModelAttribute(ATTRIBUTE_NAME) @Valid Product product, BindingResult bindingResult,
		...
		logger.info("4save  admin Order Controller  create: " + product);
